...
*/
#define PWM_DUTY_MAX 230    /* 94% duty cycle */
...
int main(void)
{
	...
	for(;;) {	  
        ...
        if (cnt>=255) {
            printf( "Vout: %5.3f V | Set point %5.3f V | Pulse %i | Integral %5.3f | DeViAtion %5.3f X%i | KP %5.3f p | KI %5.3f i| KD %5.3f d | one %i\n\r", v_load()/0.176,set_target,pwm,integral,last_error,reached,kp,ki,kd,one);
            cnt = 0;
        }
	     _delay_ms(DELAY_MS);

        if (set_target!=0 && set_target<=10 && set_target>1 && !one){ //atoi returns zero if string/ch is a character entered except for letter a, which represents value 10V
            if (!change) {
                kp = 10;
                ki = 3;
                kd = 2;
        ...
        else if (set_target<12 && set_target>10) { //11v
            if (!change) {
                kp = 10;
                ki = 0.5;
                kd = 0;
        ...
        else if (set_target<=12 && set_target>11) {
            if (!change) {
                kp = 70;
                ki = 0.2;
                kd = 0.01;

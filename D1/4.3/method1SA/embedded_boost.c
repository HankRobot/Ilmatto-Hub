...
int main(void)
{
	...
	uint16_t cnt = 0; //goes to else condition first
	...
	for(;;) {	  
        if(&str!=NULL && set_value==0){
            ...
            printf( "--> Vout: %5.3f V || Set point %5.3f\r\n", v_load()/0.176 + 0.1, set_target);
	        _delay_ms(DELAY_MS);
	        cnt++; 
            ...
            printf("\n\r-----------------------Please type in a number between 2 and 10-----------------------\n\r\n\r");
		    scanf("%s", str);
            //printf("You sent voltage setting %s lul\n\r",str);

            if (!strcmp("2",str)){
                printf("\n\r-----------------------Stay Alive Ilmatto-----------------------\n\r\n\r");
                cnt = 0;
                set_value = 0;
            }
            else{
                ...
                set_target = (set_target/100.0); //resistor error
                ...
                if (set_target!=0 && set_target<=12 && set_target>=2){ //atoi returns zero if string/ch is a character entered except for letter a, which represents value 10V
                    //printf("You sent voltage setting %s V\n\r",str);
...
void pid_control(double kp, double ki, double kd){
    ...
    double current = v_load()/0.176 + 0.1; //adc reading convert to real voltage at vout
...
/*
void setup_interrupt_rx(){
    //Set to trigger when rx is received//
    PCICR = _BV(PCIE3); //enavle port D,which is represented by number 3 
    //Enable Interrupt //
    PCMSK3 &= _BV(0);
}
*/
